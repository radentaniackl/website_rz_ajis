...
import { RefKabupatenActions } from './ref-kabupaten-actions';
import { ChevronLeft, ChevronRight } from 'lucide-react';
import Link from 'next/link';

interface RefKabupatenTableProps {
  data: Array<{ id: bigint; kode: string; nama: string; propinsiNama: string | null; ibukota: string | null; isKota: boolean; aktif: string }>;
  total: number;
  currentPage: number;
  totalPages: number;
  canEdit?: boolean;
            ...
            {data.length === 0 ? (
              ...
              data.map((item, index) => (
                <TableRow key={item.id.toString()}>
                  ...
                  <TableCell>{item.propinsiNama || '-'}</TableCell>
                  <TableCell>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {currentPage === 1 ? (
            ...
            <Link href={`/dashboard/referensi/kabupaten?page=${currentPage - 1}`}>
          ...
          <div className="flex items-center gap-1">
            {Array.from({ length: Math.min(5, totalPages) }).map((_, i) => {
              ...
              return currentPage === pageNum ? (
                ...
                <Link key={pageNum} href={`/dashboard/referensi/kabupaten?page=${pageNum}`}>
